...
export function AnimatedThemeToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`relative flex items-center justify-center w-9 h-9 rounded-full bg-white/70 hover:bg-white/90 dark:bg-neutral-800/80 dark:hover:bg-neutral-700/80 backdrop-blur-xl border border-black/10 dark:border-white/10 text-neutral-800 dark:text-neutral-100 transition-all duration-300 shadow-sm ${className}`}
    >
...
function SolarSwitch({ isDark }: { isDark: boolean }) {
  ...
  const pathLengthMoon = useTransform(scaleMoon, [0.4, 1], [0, 1]);
  const pathLengthSun = useTransform(scaleSun, [0.4, 1], [0, 1]);
